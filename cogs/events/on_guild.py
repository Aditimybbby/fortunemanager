...
class Guild(Cog):
    ...
    async def on_guild_add(self, guild):
        try:
            ...
            embed.add_field(
                name="**__Members__**",
                value=f"""<:FortuneManager_users:1226064812157571163> Members : {len(guild.members)}\n<:FortuneManager_friend:1228229690376982549> Humans : {len(list(filter(lambda m: not m.bot, guild.members)))}\n<:FortuneManager_bot:1227873668466741320> Bots : {len(list(filter(lambda m: m.bot, guild.members)))}
                """,
                inline=False
            )
            embed.add_field(
...
Threads : {len(guild.threads)}
            ...
            embed = discord.Embed(description="<:red_arrow:1247226071783440404> Prefix For This Server is `$`\n<:red_arrow:1247226071783440404> Get Started with `$help`\n<:red_arrow:1247226071783440404> For detailed guides, FAQ & information, visit our **[Support Server](https://discord.gg/fortuneleaf)**",
    color=0xff0000)
            embed.set_author(name="Thanks for adding me!", icon_url=guild.me.display_avatar.url)
            embed.set_footer(text="Powered by FortuneManager Development™", icon_url="https://cdn.discordapp.com/icons/699587669059174461/f689b4366447d5a23eda8d0ec749c1ba.png")
            if guild.icon:
                embed.set_thumbnail(url=guild.icon.url)
