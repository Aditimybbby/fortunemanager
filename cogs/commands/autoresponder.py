    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<:FortuneManager_cross:1227866668152393789> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:FortuneManager_tick:1227866641027698792> Success",
    ...
    async def _config(self, ctx):
        ...
        if not autoresponses:
            return await ctx.reply(embed=discord.Embed(
                description=f"<:FortuneManager_notify:1227866804630720565> | There are no autoresponders in {ctx.guild.name}",
                color=0x000000
            ))
