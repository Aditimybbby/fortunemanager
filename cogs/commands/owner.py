...
from core import Cog, FortuneManager, Context
import sqlite3
import os
import requests
...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
  ...
  deleted = len(deleted)
  messages = [f'<:FortuneManager_tick:1227866641027698792> | {deleted} message{" was" if deleted == 1 else "s were"} removed.']
  if deleted:
      messages.append("")
  ...
  if len(to_send) > 2000:
      await ctx.send(f"<:FortuneManager_tick:1227866641027698792> | Successfully removed {deleted} messages.", delete_after=3)
  else:
...
class Owner(commands.Cog):
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            sonu = discord.Embed(title="<:FortuneManager_notify:1227866804630720565> Access Denied", description=f"{user} is already in the staff list.", color=0x000000)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
            sonu2 = discord.Embed(title="<:FortuneManager_tick:1227866641027698792> Success", description=f"Added {user} to the staff list.", color=0x000000)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            sonu = discord.Embed(title="<:FortuneManager_notify:1227866804630720565> Access Denied", description=f"{user} is not in the staff list.", color=0x000000)
            await ctx.reply(embed=sonu, mention_author=False)
        else:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                sonu2 = discord.Embed(title="<:FortuneManager_tick:1227866641027698792> Success", description=f"Removed {user} from the staff list.", color=0x000000)
            await ctx.reply(embed=sonu2, mention_author=False)
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="<:FortuneManager_staff:1228227884481515613> FortuneManager Staffs", description=f"\n{staff_display}", color=0x000000)
    ...
    async def _slist(self, ctx):
        ...
        embeds = DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of FortuneManager [{len(self.client.guilds)}]",
            color=0x000000,
    ...
    @commands.command(name="FortuneManager.restart", help="Restarts the client.")
    @commands.is_owner()
    async def _restart(self, ctx: Context):
        await ctx.reply("Restarting FortuneManager...")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        embeds = DescriptionEmbedPaginator(
            entries=entries,
            title=f"FortuneManager Owners [{len(nplist)}]",
            description="",
            per_page=10,
    ...
    async def dm(self, ctx, user: discord.User, *, message: str):
        ...
        try:
            ...
            await ctx.send(f"<:FortuneManager_tick:1227866641027698792> | Successfully Sent a DM to **{user}**")
        except discord.Forbidden:
    ...
    async def change_nickname(self, ctx, *, name: str = None):
        ...
        try:
            ...
            if name:
                await ctx.send(f"<:FortuneManager_tick:1227866641027698792> | Successfully changed nickname to **{name}**")
            else:
                await ctx.send("<:FortuneManager_tick:1227866641027698792> | Successfully removed nickname")
        ...
        if member:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<:FortuneManager_tick:1227866641027698792> | **{member.name}** has been successfully banned from {ctx.guild.name} by the Bot Owner.",
                    color=0x000000)
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:FortuneManager_notify:1227866804630720565> I do not have permission to ban **{member.name}** in this guild.",
                    color=0x000000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:FortuneManager_notify:1227866804630720565> An error occurred while banning **{member.name}**.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False, delete_after=5)
                await ctx.message.delete()
        ...
        if user:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<:FortuneManager_tick:1227866641027698792> | **{user.name}** has been successfully unbanned from {ctx.guild.name} by the Bot Owner.",
                    color=0x000000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:FortuneManager_notify:1227866804630720565> I do not have permission to unban **{user.name}** in this guild.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False)
            except discord.HTTPException:
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:FortuneManager_notify:1227866804630720565> An error occurred while unbanning **{user.name}**.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False)
        else:
    ...
    async def _owner_help(self, ctx):
        ...
        embed = Embed(title="Owner Commands",
                      description="`staffadd` ,   `staffremove` ,   `stafflist` , `slist` ,   `getinvite <guild-id>` ,   `getguild <channel-id>` ,   `mutual <user>` ,   `guildban <guild_id> <user_id>` ,   `guildunban <guild_id> <user_id>` ,   `FortuneManager.restart` ,   `servertour` ,   `forcepurgebots` , `forcepurgeuser` ,   `ownerban` , `bdg add <user> <badge>` ,   `bdg remove <user> <badge>` ,   `global <subcommand>` ,   `np <subcommand>` ,   `autonp <subcommand>`",
                      color=0x000000)
        await ctx.send(embed=embed)
