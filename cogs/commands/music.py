...
spotify_api = SpotifyAPI(client_id="ac2b614ca5ce46a18dfd1d3475fd6fd9", client_secret=os.getenv("CLIENT_SECRET", ""))
...
class Music(commands.Cog):
    ...
    async def play_source(self, ctx, query):
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(embed=discord.Embed(description=f"<:add_white:1205381455505661952> Added playlist [{tracks.name}](https://discord.gg/fortuneleaf) with **{len(tracks.tracks)} songs** to the queue.", color=0x000000))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"<:add_white:1205381455505661952> Added [{track.title}](https://discord.gg/fortuneleaf) to the queue.", color=0x000000))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"<:add_white:1205381455505661952> Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/fortuneleaf)** to the queue.", color=0x000000))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"<:add_white:1205381455505661952> Added all tracks from album **[{album_info['name']}](https://discord.gg/fortuneleaf)** to the queue.", color=0x000000))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
