...
class Fun(commands.Cog):
  ...
  def __init__(self, bot):
    self.bot = bot
    self.giphy_token = os.getenv("TOKEN", "")
    ...
    async with aiohttp.ClientSession() as session:
      try:
        async with session.get(f"http://ip-api.com/json/{ip}") as response:
          ...
          embed.set_footer(
            text=f'Made by FortuneManager Development™',
...
############################
  ...
  async def fake_ban(self, ctx, user: discord.Member):
    embed = discord.Embed(
      ...
      description=f"<:FortuneManager_tick:1227866641027698792> | {user.mention} has been successfully banned",
      color=0x000000
    )
    embed.set_footer(
