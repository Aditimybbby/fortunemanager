...
class OnOrOff(BasicView):
    ...
    @discord.ui.button(label="DM me", emoji="<:FortuneManager_tick:1227866641027698792>", custom_id='Yes', style=discord.ButtonStyle.green)
    async def dare(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.value = 'Yes'
        await interaction.response.defer()
        self.stop()

    @discord.ui.button(label="Don't DM", emoji="<:FortuneManager_cross:1227866668152393789>", custom_id='No', style=discord.ButtonStyle.danger)
...
class afk(commands.Cog):
    ...
    async def afk(self, ctx, *, reason=None):
        ...
        if any(invite in reason.lower() for invite in ['discord.gg', 'gg/']):
            emd = discord.Embed(description="<:FortuneManager_notify:1227866804630720565> | You can't advertise Serve Invite in the AFK reason", color=0x0c0606)
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await test.delete()
            af = discord.Embed(title='<:FortuneManager_tick:1227866641027698792> Success', 
