...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
    ...
    if len(to_send) > 2000:
        await ctx.send(f"<:FortuneManager_tick:1227866641027698792> | Successfully removed {deleted} messages.", delete_after=7)
    else:
...
class Moderation(commands.Cog):
                  ...
                  if interaction.guild.me.guild_permissions.manage_roles:
                    ...
                    description=f"Unlocking all channels in {ctx.guild.name} .")
                      ...
                      await interaction.channel.send(
                              content=f"<:FortuneManager_tick:1227866641027698792> | Successfully Unlocked {a} Channels")   
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                           "<:FortuneManager_notify:1227866804630720565> | It seems I'm missing the necessary permissions. Please grant me the `manage roles` permissions and try again.",
                              embed=None,
                                  view=None)
              else:
                    ...
                    description=f"Locking all channels in {ctx.guild.name}...")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:FortuneManager_tick:1227866641027698792> | Successfully locked {a} Channels")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                    ...
                    description=f"Hiding all channels in {ctx.guild.name} ...")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:FortuneManager_tick:1227866641027698792> | Successfully Hidden {a} Channel(s) .")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                    ...
                    description=f"Unhiding all channels in {ctx.guild.name} .")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:FortuneManager_tick:1227866641027698792> | Successfully Unhidden {a} Channel(s) .")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
  ...
  async def _prefix(self, ctx: commands.Context, prefix: str):
      if not prefix:
          await ctx.reply(embed=discord.Embed(title="<:FortuneManager_cross:1227866668152393789> Error",
      ...
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          embed1=discord.Embed(title="<:FortuneManager_tick:1227866641027698792> Success",
        ...
        if len(found_emojis) != 0:
            ...
            for emoji_id in found_emojis:
                try:
                    ...
                    continue  
            await init_message.delete()
            return await ctx.reply(f"<:FortuneManager_tick:1227866641027698792> | Successfully deleted {delete_count}/{len(found_emojis)} emoji(s).")
  ...
  async def roleicon(self, ctx: commands.Context, role: discord.Role, *, icon: Union[discord.Emoji, discord.PartialEmoji, str] = None):
    ...
    if isinstance(icon, discord.Emoji) or isinstance(icon, discord.PartialEmoji):
        ...
        try:
            ...
            await role.edit(display_icon=image_data)
            success_embed = discord.Embed(
                description=f"<:FortuneManager_tick:1227866641027698792> | Successfully changed the icon for {role.mention}.",
                color=self.color
            )
            return await ctx.reply(embed=success_embed, mention_author=False)
        except Exception as e:
      ...
      if interaction.user == ctx.author:
        if interaction.guild.me.guild_permissions.ban_members:
          ...
          async for idk in interaction.guild.bans(limit=None):
            ...
            a += 1
          await interaction.channel.send(
            content=f"<:FortuneManager_tick:1227866641027698792> Successfully Unbanned {a} Members")
        else:
          await interaction.response.edit_message(
            content=
