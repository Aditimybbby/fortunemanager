...
class Ban(commands.Cog):
    ...
    async def ban(self, ctx, user: discord.User, *, reason=None):
        ...
        try:
            await user.send(f"<a:FortuneManager_WarnFlash:1272569018183843874> You have been banned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
